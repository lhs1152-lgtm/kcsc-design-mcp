...
#: 하위호환 별칭 — 옛 이름으로 확정해 둔 트리도 확정으로 본다.
#
# ★이 별칭을 지우면 그 이름으로 확정된 기존 트리가 전부 '미확정' 으로 떨어지고,
#   흐름·엑셀에 경고가 붙으며 `design_stamp(all_confirmed=True)` 의 대상이 0건이 된다.
#   쓰는 곳이 남아 있는 한 지우지 말 것.
VERIFY_OK_ALIASES = ("사장님확정",)

#: 새로 쓸 때 권하는 값 — 오류 메시지에는 이것만 보여 준다(별칭을 권하지 않기 위해).
VERIFY_STATES_CANON = (VERIFY_OK, VERIFY_MACHINE, "draft", "예제", "수정필요")

#: `검증:` 에 실제로 받아 주는 값. 별칭을 포함한다.
...
def is_confirmed(t: dict) -> bool:
    """설계자가 확정한 트리인가. 옛 이름으로 적힌 것도 확정으로 본다."""
