...
#
#  강구조(KDS 14 3x)와 교량 강교(KDS 24 14 3x)는 **설계법 계열이 다르다.**
#    교량    한계상태설계법  KDS 24 14 31 강교   (+ 하중조합 24 12 11, 설계하중 24 12 21)
#    교량    허용응력설계법  KDS 24 14 30 강교
#    강구조  하중저항계수설계법 KDS 14 31 xx / 허용응력설계법 KDS 14 30 xx
#  개념으로는 LRFD 도 한계상태설계법의 하나지만 **기준 이름으로는 별개 계열**이라,
#  아무 말 없이 섞으면 교량 설계자에게 다른 계열 기준을 내주게 된다. 하중조합부터 다르다.
#  그래서 분야를 명시하지 않으면 교량으로 둔다.
#
#  ★"건축"이라 부르지 않는다(2026-08-24 정정).
#    KDS 14 3x 의 이름은 「**강구조** 설계 일반사항」·「**강구조** 부재 설계기준」처럼
#    전부 '강구조'이고, 1.1 적용범위에도 건축이라는 말이 없다("강구조 부재의 해석 및
#    설계방법"). 실제로 교량도 이 기준을 쓴다 — KDS 24 14 31 4.1.4.2 가 압축부재 산정을
#    KDS 14 31 10 (4.2) 로 넘긴다. 같은 14 계열에 건축 시공 기준(KCS 14 31 70 데크플레이트
#    등)이 섞여 있는 것은 맞지만, 설계기준 쪽을 '건축'으로 못 박으면 **교량에서 못 쓰는
#    것처럼 읽힌다.** 실제로 그렇게 오해한 답이 나왔다.
DOMAIN_DEFAULT = "교량"
...
#: 옛 이름 → 지금 이름. 이미 `domain='건축'` 으로 쓰던 곳이 계속 돌게 한다.
DOMAIN_ALIASES = {
    "건축": "강구조",
}
